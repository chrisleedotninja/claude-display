...
- Options considered: (a) render at top level — treat the event as if it had no parent linkage, recording the subagent as an ordinary top-level record; (b) drop the event silently; (c) buffer the event until the parent registers, then re-attribute.
- Chosen: **A** — render at top level. When an event arrives with a `parent_id` that is not present in server state, the server records the subagent at top level (the same record shape it would create for an ordinary top-level instance with that `id`).
- Rationale: the server is in-memory per ADR 0001, and a server restart loses every parent registration — so the orphan case is not exotic, it is the routine first hook fire after every restart. Buffering (c) adds per-event TTL bookkeeping for a transient state that the parent's next hook fire resolves naturally. Dropping silently (b) swallows visibility the user explicitly asked for in [001]'s PRD. Top-level rendering degrades gracefully: the subagent's card is visible immediately, and once the parent re-registers and its next event arrives, future subagent fires nest correctly. This matches [007]'s acceptance criterion that a subagent without a known parent does not crash the server, with the chore deciding whether it renders as a top-level card or is dropped — and we choose top-level over dropped.
